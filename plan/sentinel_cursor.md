...
> **Fuente viva en el repo:** la versión actualizada con el estado de implementación está en la raíz: [`CURSOR.md`](../CURSOR.md). Este fichero en `plan/` se mantiene como copia de referencia junto al PRD y la spec técnica.
...
- `tailwindcss` + `shadcn/ui` — sistema de diseño oscuro
- `recharts` — gráficas en metric cards
...
- Estado de proyectos en `src/store/projectStore.ts` (Zustand)

---

## Stack API (`apps/api`)

- Node.js 20 LTS + Fastify 4 + TypeScript strict
- `simple-git` — operaciones Git (clone, pull, checkout)
- `dockerode` — Docker Engine API
- `node-pty` — ejecución de scripts con output en streaming real
- `@octokit/rest` — GitHub API
- `lowdb` — persistencia JSON simple (sin base de datos externa)
...
- Persistencia en `lowdb` — sin base de datos externa
...
Hook de SSE disponible en `src/hooks/useSSE.ts`.
...
1. Monorepo base con pnpm workspaces + shared-types
2. Sentinel API: estructura Fastify + auth middleware + lowdb store
3. `GitService` — clone y pull
4. `DockerService` — compose up/down + stream de logs via SSE
5. Sentinel UI: canvas base con React Flow + dot grid background
...
- Sin base de datos externa — `lowdb` JSON es suficiente para este caso de uso
